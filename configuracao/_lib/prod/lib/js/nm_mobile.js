...
function bootstrapMobile() {
    ...
    if (isMobile() && app.improvements) {
        ...
        tb_show = function tb_show(a, b, c) {
            var $tb_remove = function $tb_remove() {};
            if (typeof(tb_remove) === 'function') {
                console.log('sav\'d');
                $tb_remove = tb_remove;
            }
            tb_remove = function tb_remove(a, b, c) {
                window.history.go(-1);
                tb_remove = $tb_remove;
            };
            // console.log('teste');
            if (!$('#TB_iframeContent')[0]) {
                var frameHTML = '<div id="TB_iframeContent_wrapper"><iframe frameborder="0" hspace="0" id="TB_iframeContent" name="TB_iframeContent700"></iframe><div>';
            ...
            tb_frame_wrapper.toggleModalPane(true);
            toggleToolbar();
            if (typeof(tb_remove) === 'function') {
                console.log('sav\'d');
                $tb_remove = tb_remove;
